...
/*
* ConsumerApp.java
This source file contains the basic code needed to consume records from
one or many Apache Kafka partitions. As discussed in Module 5,
 the method for consuming records from a specific list of partitions is assign()
 and it accepts TopicPartition objects representing the partitions of interest.
 The code to enable partition assignment is provided with some helper
 functions to list the partitions assigned. The records retrieved from the cluster
 in the poll loop are printed to the output.
